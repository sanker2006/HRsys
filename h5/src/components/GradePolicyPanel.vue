  ...
  <section v-if="policy" class="policy-panel">
    ...
    <div class="grade-grid">
      <div v-for="grade in grades" :key="grade" class="grade-item" :class="grade.toLowerCase()">
        <span>{{ grade }}级</span><b>{{ policy.counts?.[grade] || 0 }}</b>
      </div>
    </div>
...
.grade-item span, .grade-item b { display: block; }
.grade-item span { font-size: 11px; }.grade-item b { margin-top: 3px; font-size: 18px; }
