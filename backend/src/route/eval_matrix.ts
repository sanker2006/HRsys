...
router.get('/:batchId', async (ctx: Context) => {
  ...
  let matrix = await EvalMatrixModel.findByBatchId(batchId);
  if (matrix.length === 0) {
    await EvalMatrixModel.initDefaultMatrix(batchId);
    matrix = await EvalMatrixModel.findByBatchId(batchId);
  }
...
router.put('/:batchId', admin, async (ctx: Context) => {
  ...
  await EvalMatrixModel.saveMatrix(batchId, rows);
  if (peer_cross_dept !== undefined) {
    await BatchModel.update(batchId, { peer_cross_dept: peer_cross_dept ? 1 : 0 });
  }
  const matrix = await EvalMatrixModel.findByBatchId(batchId);
  const updatedBatch = (await BatchModel.findById(batchId))!;
...
router.post('/:batchId/reset', admin, async (ctx: Context) => {
  ...
  const batch = await BatchModel.findById(batchId);
  if (!batch) return fail(ctx, '批次不存在', -1, 404);
  await EvalMatrixModel.deleteByBatchId(batchId);
  await EvalMatrixModel.initDefaultMatrix(batchId);
  await BatchModel.update(batchId, { peer_cross_dept: 0 });
  const matrix = await EvalMatrixModel.findByBatchId(batchId);
