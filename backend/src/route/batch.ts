...
router.get('/', async (ctx: Context) => {
  ...
  const batches = await BatchModel.findAll({ status, keyword });
  const list = await Promise.all(batches.map(async b => {
    const stats = await RelationModel.getStats(b.id);
    return { ...b, ...stats };
  }));
  success(ctx, list);
});
...
router.get('/:id', async (ctx: Context) => {
  ...
  const stats = await RelationModel.getStats(id);
...
router.post('/', admin, async (ctx: Context) => {
  ...
  const batch = await BatchModel.create({ name, period, start_time, end_time, peer_cross_dept });
  success(ctx, batch, '创建成功');
});

router.put('/:id', admin, async (ctx: Context) => {
  const id = parseInt(ctx.params.id);
  const existing = await BatchModel.findById(id);
  if (!existing) return fail(ctx, '批次不存在', -1, 404);
  await BatchModel.update(id, ctx.request.body as any);
  const updated = await BatchModel.findById(id);
...
router.post('/:id/close', admin, async (ctx: Context) => {
  ...
  await BatchModel.update(id, { status: 'closed' });
  success(ctx, await BatchModel.findById(id), '已结束');
});
